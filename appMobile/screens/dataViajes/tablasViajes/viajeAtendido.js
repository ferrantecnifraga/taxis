  ...
  const agregarViajes = async (viajes) => {
    ...
    try {
      arrayDetalles.forEach(async (e) => {
        crear = await Calendar.createEventAsync(calendario, e);
        arrayIDS.push(crear);
      });
    } catch (error) {
      Alert.alert(
  ...
  return (
    <View>
      <Card containerStyle={styles.cardContainer}>
        ...
        <Text style={styles.titleMsg}>¡Gracias!</Text>
        <Text style={styles.textMsg}>
          Confirmaste el servicio, en viajes programados puedes checar estatus y
          detalles.
        </Text>
        <Button
